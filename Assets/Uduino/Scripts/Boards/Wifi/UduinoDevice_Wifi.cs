        ...
        public override bool WriteToArduinoLoop()
        {
            ...
            lock (writeQueue)
            {
                ...
                try
                {
                    ...
                    Debug.Log(message + " sent to [" + (name != "" ? name : identity) + "]");
                }
                catch (Exception e)
                {
                    //Debug.Log("Impossible to send the message" + message + "</color> to <color=#2196F3>[" + wifiSetting.ip + ":"+ wifiSetting.port+ "]</color>: " + e);
